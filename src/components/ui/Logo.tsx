...
export default function Logo({ className, isDark }: LogoProps) {
  // Ajustamos los colores según si estemos en un fondo oscuro forzado (Footer) o adaptable (Header)
  const textColor = isDark 
    ? "text-brand-50 group-hover:text-brand-200" 
    : "text-brand-900 group-hover:text-brand-600";
    
  // Subtitle will be the beautiful Mauve accent
  ...
  const iconBg = isDark 
    ...
    : "bg-white/50 border-brand-200 shadow-sm";
  ...
  return (
    <Link href="/" className={`flex items-center gap-3 group ${className || ""}`}>
      <div className={`relative w-10 h-10 md:w-11 md:h-11 rounded-full flex items-center justify-center border transition-transform duration-500 group-hover:scale-105 ${iconBg}`}>
         ...
         <Sparkles className="absolute -top-1 -right-1 w-3 h-3 md:w-4 md:h-4 text-teal-400 opacity-80" />
      </div>
