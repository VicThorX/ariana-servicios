...
import { 
  ...
  Briefcase, ShoppingCart, Info, ShieldAlert
} from "lucide-react";
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (response.success) {
      if (sendMethod === 'whatsapp') {
        const wppText = `Hola Ariana! Quisiera solicitar presupuesto de limpieza corporativa:\n\n` +
          `👤 *Nombre:* ${name} (${role})\n` +
          `📧 *Email:* ${email}\n` +
          `📱 *Teléfono:* ${phone}\n` +
          `🏢 *Establecimiento:* ${estType} (${address})\n` +
          `⏱️ *Frecuencia:* ${frequency}\n` +
          `🕒 *Horario:* ${schedule}\n` +
          `🗓️ *Fecha inicio:* ${start}\n` +
          (estType.toLowerCase() === "consorcio" ? `🏛️ *Administración:* ${adminName} (${adminContact})\n` : "") +
          `📝 *Detalles:* ${message}`;

        const wppUrl = `https://wa.me/5492235220338?text=${encodeURIComponent(wppText)}`;
      ...
      setFeedback({ 
        ...
        message: sendMethod === 'whatsapp' 
          ? "¡Presupuesto registrado! Se abrió WhatsApp para continuar la gestión directamente con nuestro asesor." 
          : response.message 
      });
    ...
    <section id="contacto" className="py-24 bg-brand-50 dark:bg-slate-950 relative overflow-x-clip border-t border-brand-100 dark:border-slate-900">
      ...
      <div className="container mx-auto px-6 max-w-6xl relative z-10">
        ...
        <div className="grid lg:grid-cols-12 gap-12 items-start">
            ...
            <div className="bg-white/80 dark:bg-slate-900 p-8 rounded-[2.5rem] border border-brand-100 dark:border-slate-800 shadow-xl backdrop-blur-md">
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-start gap-4 group">
                  ...
                  <div>
                    <p className="text-[11px] font-bold tracking-wider uppercase text-slate-500 dark:text-slate-400 mb-0.5">Asesoramiento Comercial</p>
                    <a
                      href="https://wa.me/5492235220338?text=Hola%20Ariana!%20Vengo%20de%20la%20pagina%20web%20y%20me%20gustar%C3%ADa%20hacer%20una%20consulta%20comercial."
                      target="_blank"
                    ...
                    <span className="inline-block mt-1 px-2 py-0.5 rounded text-[10px] font-semibold bg-amber-100/70 dark:bg-amber-950/40 text-amber-800 dark:text-amber-300 border border-amber-200/50">
                      Exclusivo Presupuestos (No CVs)
                    </span>
                  </div>
                </div>
                ...
                {/* 1. Presupuestos */}
                <div className="flex items-start gap-4 group">
                  <div className="w-10 h-10 shrink-0 rounded-xl bg-brand-50 dark:bg-slate-800 flex items-center justify-center text-brand-650 dark:text-brand-400 border border-brand-100 dark:border-slate-700 group-hover:scale-105 transition-transform">
                    <Building2 className="w-4 h-4" />
                  </div>
                  <div>
                    <p className="text-[11px] font-bold uppercase text-slate-500 dark:text-slate-400 mb-0.5">Cotizaciones Corporativas</p>
                    <a href="mailto:presupuestos@arianaservicios.com.ar" className="text-sm font-bold text-slate-800 dark:text-white hover:text-brand-600 transition-colors break-all block">
                      presupuestos@arianaservicios.com.ar
                    </a>
                  </div>
                </div>
                ...
                {/* 2. Ventas e Insumos */}
                <div className="flex items-start gap-4 group">
                  <div className="w-10 h-10 shrink-0 rounded-xl bg-teal-50 dark:bg-slate-800 flex items-center justify-center text-teal-600 dark:text-teal-400 border border-teal-100 dark:border-slate-700 group-hover:scale-105 transition-transform">
                    <ShoppingCart className="w-4 h-4" />
                  </div>
                  <div>
                    <p className="text-[11px] font-bold uppercase text-slate-500 dark:text-slate-400 mb-0.5">Ventas e Insumos</p>
                    <a href="mailto:ventas@arianaservicios.com.ar" className="text-sm font-bold text-slate-800 dark:text-white hover:text-brand-600 transition-colors break-all block">
                      ventas@arianaservicios.com.ar
                    </a>
                  </div>
                </div>

                {/* 3. Recepción de CV / RRHH */}
                <div className="flex items-start gap-4 group">
                  <div className="w-10 h-10 shrink-0 rounded-xl bg-purple-50 dark:bg-slate-800 flex items-center justify-center text-purple-600 dark:text-purple-400 border border-purple-100 dark:border-slate-700 group-hover:scale-105 transition-transform">
                    <Briefcase className="w-4 h-4" />
                  </div>
                  <div>
                    <p className="text-[11px] font-bold uppercase text-slate-500 dark:text-slate-400 mb-0.5">Envío de CV / Selección</p>
                    <a href="mailto:cv@arianaservicios.com.ar" className="text-sm font-bold text-slate-800 dark:text-white hover:text-brand-600 transition-colors break-all block">
                      cv@arianaservicios.com.ar
                    </a>
                    <Link 
                      href="/trabaja-con-nosotros" 
                      className="inline-flex items-center gap-1 text-[11px] font-bold text-brand-600 dark:text-brand-400 hover:underline mt-1"
                    >
                      Ir al Formulario de Selección ➔
                    </Link>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-4 group">
                  <div className="w-10 h-10 shrink-0 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700 group-hover:scale-105 transition-transform">
                    <Info className="w-4 h-4" />
                  </div>
                  <div>
                    <p className="text-[11px] font-bold uppercase text-slate-500 dark:text-slate-400 mb-0.5">Consultas Generales</p>
                    <a href="mailto:info@arianaservicios.com.ar" className="text-sm font-bold text-slate-800 dark:text-white hover:text-brand-600 transition-colors break-all block">
                ...
                {/* Ubicación */}
                <div className="flex items-start gap-4 pt-2 border-t border-brand-50 dark:border-slate-800">
                  <div className="w-10 h-10 shrink-0 rounded-xl bg-amber-50 dark:bg-slate-800 flex items-center justify-center text-amber-600 dark:text-amber-400 border border-amber-100 dark:border-slate-700">
                    <MapPin className="w-4 h-4" />
                  </div>
                  <div>
                    <p className="text-[11px] font-bold uppercase text-slate-500 dark:text-slate-400 mb-0.5">Zona de Cobertura</p>
                    <p className="text-sm font-bold text-slate-850 dark:text-white">Mar del Plata y alrededores</p>
                  </div>
                </div>
          ...
          >
            {/* Banner aclaratorio para candidatos de empleo */}
            <div className="mb-6 p-4 rounded-2xl bg-amber-50/70 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900/50 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs">
              <div className="flex items-center gap-2 text-amber-800 dark:text-amber-300 font-semibold">
                <Briefcase className="w-4 h-4 shrink-0 text-amber-600 dark:text-amber-400" />
                <span>¿Buscás trabajo o querés enviar tu CV?</span>
              </div>
              <Link
                href="/trabaja-con-nosotros"
                className="px-3.5 py-1.5 rounded-xl bg-amber-600 hover:bg-amber-700 text-white font-bold transition-all shrink-0 flex items-center gap-1 shadow-sm"
              >
                Cargar CV en Selección ➔
              </Link>
            </div>
            ...
            <form ref={formRef} className="space-y-6" onSubmit={handleSubmit} noValidate>
              ...
              <div className={step === 3 ? "space-y-6" : "hidden"}>
                ...
                {/* Selección del método de envío del presupuesto */}
                <div className="space-y-3 bg-brand-50/40 dark:bg-slate-850 p-5 rounded-2xl border border-brand-100 dark:border-slate-800">
                  <label className="text-xs sm:text-sm font-bold text-slate-850 dark:text-white block">
                    ¿Cómo preferís recibir/enviar la propuesta? *
                  </label>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <button
                      ...
                      className={`p-3.5 rounded-xl border text-left flex items-center gap-3 transition-all ${
                        sendMethod === 'email'
                          ? 'border-brand-500 bg-brand-50/90 dark:bg-brand-950/40 text-brand-700 dark:text-brand-300 ring-2 ring-brand-500/20 font-bold'
                          : 'border-brand-200 dark:border-slate-750 bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-400 hover:border-brand-300'
                      }`}
                    >
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${sendMethod === 'email' ? 'bg-brand-500 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-500'}`}>
                        <Mail className="w-4 h-4" />
                      </div>
                      <div>
                        <p className="text-xs font-bold leading-tight">Vía Email</p>
                        <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">Recibir propuesta por correo formal</p>
                      </div>
                    </button>
                    ...
                    <button
                      ...
                      className={`p-3.5 rounded-xl border text-left flex items-center gap-3 transition-all ${
                        sendMethod === 'whatsapp'
                          ? 'border-[#25D366] bg-teal-50/90 dark:bg-emerald-950/40 text-teal-800 dark:text-emerald-300 ring-2 ring-[#25D366]/20 font-bold'
                          : 'border-brand-200 dark:border-slate-750 bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-400 hover:border-[#25D366]/50'
                      }`}
                    >
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${sendMethod === 'whatsapp' ? 'bg-[#25D366] text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-500'}`}>
                        <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor">
                          <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51a12.8 12.8 0 0 0-.57-.015c-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.82 9.82 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 0 0-3.48-8.413Z"/>
                        </svg>
                      </div>
                      <div>
                        <p className="text-xs font-bold leading-tight">Vía WhatsApp</p>
                        <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">Atención inmediata con asesor</p>
                      </div>
                    </button>
                  </div>
                </div>
              ...
              <div className="flex gap-4 pt-6 border-t border-brand-100 dark:border-slate-800">
                ...
                {step < 3 ? (
                  ...
                  <button
                    ...
                    type="submit"
                    disabled={isSubmitting}
                    className={`flex-1 py-4 text-white font-bold rounded-2xl transition-all flex items-center justify-center gap-2 shadow-lg disabled:opacity-50 disabled:cursor-not-allowed ${
                      sendMethod === 'whatsapp'
                        ? 'bg-[#25D366] hover:bg-[#20bd5a] shadow-[#25D366]/20'
                        : 'bg-brand-500 hover:bg-brand-600 dark:bg-brand-650 dark:hover:bg-brand-700 shadow-brand-500/10'
                    }`}
                  >
                    {isSubmitting ? (
                      <>
