...
export default function Products() {
  const products = [
    ...
    },
    {
      ...
      description: "Ambientadores y desodorizantes lavanda orientados a no ser reactivos. Porque un verdadero espacio limpio, se respira limpio y no tóxico.",
  ...
  return (
    <section id="productos" className="py-24 bg-brand-50/20 dark:bg-brand-950/40 overflow-x-clip relative">
      <div className="absolute top-0 right-0 w-[800px] h-[800px] bg-brand-200/20 dark:bg-brand-800/20 blur-[150px] rounded-full pointer-events-none -translate-y-1/3" />
      ...
      <div className="container mx-auto px-6 max-w-6xl relative z-10">
        ...
        <div className="grid lg:grid-cols-3 gap-10">
          {products.map((product, index) => (
            ...
            >
              <div className="relative h-72 w-full overflow-hidden bg-brand-50 dark:bg-brand-950/50">
                <Image
                  src={product.image}
                  alt={product.title}
                  fill
              ...
              <div className="p-8 flex-1 flex flex-col relative z-20">
                <div className="w-12 h-12 bg-brand-50 dark:bg-brand-800/80 rounded-2xl flex items-center justify-center mb-5 border border-brand-100 dark:border-brand-700 group-hover:bg-brand-100 dark:group-hover:bg-brand-700 group-hover:scale-110 transition-all duration-500">
                  {product.icon}
                </div>
                <h3 className="text-2xl font-bold text-slate-800 dark:text-brand-50 mb-3">{product.title}</h3>
                <p className="text-slate-600 dark:text-brand-200 leading-relaxed font-medium">{product.description}</p>
              </div>
            </motion.div>
          ))}
        </div>
